...
**P1-20 SOURCE-AWARE V2 GATE IMPLEMENTED; LIVE RE-ACCEPTANCE PENDING (26-Aug-2026)**
...
- generated TP validation checks DOCX integrity, exact
  ...
  selling facts with punctuation-independent decimal comparison, prohibited
  internal labels, approved RAG provenance, masked-client alias removal across
  all Word stories, embedded-image relationship integrity and source-aware
...
- the former fixed 16-inline-shape rule was retired on 26-Aug because genuine
  vendor documents have different valid diagram counts; template branding is
  no longer allowed to inflate the comparison
- the adapter now requires auditable masked client code, real-name aliases and
  `mask_client=true`, maps those fields to the frozen Builder's existing
  multipart contract, and quarantines output if any alias survives
- the reviewed Builder artifact remains rejected because it was generated by a
  generic fixture without the required identity metadata and contains real
  customer references; live v2 regeneration and visual acceptance are pending
...
**Production exit gates / P1-20 live v2 re-acceptance**
...
- the P1-20 automatic gate is now `p1-20.tp-source-aware.v2`. It normalizes
  displayed money before exact `Decimal` comparison, verifies image-package
  relationships, compares main-document drawings to detected source graphics,
  and checks every Word story for surviving real-client aliases. The invalid
  fixed 16-inline-shape floor has been removed.
- focused P1-19/P1-20 tests passed 20/20; the broader P1-19 through P1-22
  offline suite passed 42 with one expected live skip; the complete image test
  suite passed 109 with two expected opt-in live skips against a disposable
  PostgreSQL 16/pgvector database initialized with the P1-02 audit schema.
- live acceptance is not yet claimed. The Builder must regenerate from the
  genuine hash-pinned source through the Orchestrator identity contract, the v2
  report must pass, and a human must approve the rendered pages.
