...
ALTER TABLE quotes ADD COLUMN IF NOT EXISTS validation_status TEXT NOT NULL DEFAULT 'NOT_VALIDATED';
ALTER TABLE quotes ADD COLUMN IF NOT EXISTS validated_at TIMESTAMPTZ;
ALTER TABLE quotes ADD COLUMN IF NOT EXISTS validated_by TEXT;
...
ALTER TABLE quote_review_queue ADD COLUMN IF NOT EXISTS quote_id BIGINT REFERENCES quotes(quote_id);

CREATE TABLE IF NOT EXISTS quote_validation_results (
    ...
    computed_subtotal   NUMERIC(16,2) NOT NULL,
    computed_vat        NUMERIC(16,2) NOT NULL,
    computed_total      NUMERIC(16,2) NOT NULL,
    issues              JSONB NOT NULL,
    validated_by        TEXT NOT NULL,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    CHECK (status IN ('VALIDATED','BLOCKED','NEEDS_REVIEW'))
);
CREATE INDEX IF NOT EXISTS idx_quote_validation_history
    ON quote_validation_results (quote_id, validation_id DESC);
"""
...
class PostgresQuoteRepository:
    ...
    def validate_quote(
        ...
        *,
        ...
        """Validate persisted claims without changing vendor-supplied values."""
        with psycopg.connect(self.dsn) as conn:
            quote = conn.execute(
                "SELECT q.opp_id,q.response_id,q.currency,q.currency_detected,q.status,q.is_current,"
                "q.warning,q.validation_status FROM quotes q WHERE q.quote_id=%s FOR UPDATE",
                (quote_id,),
            ).fetchone()
            if not quote:
            ...
            rows = conn.execute(
                "SELECT line_no,quantity,unit_price,line_total FROM quote_line_items "
                "WHERE quote_id=%s ORDER BY line_no",
                (quote_id,),
            ).fetchall()
            lines = tuple(ValidationLine.create(*row) for row in rows)
            ...
            validation_id = conn.execute(
                ...
                "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING validation_id",
                (
                    quote_id,
                    result.engine_version,
                    result.status,
                    result.currency,
                    _json(policy.as_dict()),
                    _json(claims.as_dict()),
                    result.computed_subtotal,
                    result.computed_vat,
            ...
            conn.execute(
                "UPDATE quotes SET validation_status=%s,validated_at=now(),validated_by=%s "
                "WHERE quote_id=%s",
                (result.status, actor, quote_id),
            )

            review_id = None
            if result.may_proceed:
                conn.execute(
                    "UPDATE quote_review_queue SET status='RESOLVED',resolved_at=now(),"
                    "resolved_by=%s,resolution='deterministic validation passed' "
                    "WHERE response_id=%s AND status='OPEN' "
                    "AND error_code IN ('QUOTE_VALIDATION_BLOCKED','QUOTE_VALIDATION_REVIEW')",
                    (actor, response_id),
                )
            else:
                review_code = (
                    ...
                    else "QUOTE_VALIDATION_REVIEW"
                )
                reason = "; ".join(f"{item.code}: {item.message}" for item in result.issues)
                review_id = conn.execute(
                    "INSERT INTO quote_review_queue "
                    "(response_id,opp_id,quote_id,error_code,reason,status) "
                    "VALUES (%s,%s,%s,%s,%s,'OPEN') "
                    "ON CONFLICT (response_id) DO UPDATE SET quote_id=EXCLUDED.quote_id,"
                    "error_code=EXCLUDED.error_code,reason=EXCLUDED.reason,status='OPEN',"
                    "resolved_at=NULL,resolved_by=NULL,resolution=NULL RETURNING review_id",
                    (response_id, opp_id, quote_id, review_code, reason[:4000]),
                ).fetchone()[0]
            ...
            audit_payload = {
                ...
                "computed_subtotal": str(result.computed_subtotal),
                "computed_vat": str(result.computed_vat),
                "computed_total": str(result.computed_total),
                "claims": claims.as_dict(),
                "issues": issues,
            }
            action = {
                "VALIDATED": "quote_validation_passed",
                "BLOCKED": "quote_validation_blocked",
                "NEEDS_REVIEW": "quote_validation_review_required",
            }[result.status]
            audit(
                conn,
                actor,
                ...
                new=_json(audit_payload),
                reason=", ".join(item.code for item in result.issues) or None,
            )
            conn.commit()
    ...
    def latest_validation(self, quote_id: int) -> dict[str, Any] | None:
        with psycopg.connect(self.dsn) as conn:
            row = conn.execute(
                "SELECT validation_id,status,currency,computed_subtotal,computed_vat,"
                "computed_total,issues,policy_snapshot,claims_snapshot,engine_version,"
                "validated_by,created_at FROM quote_validation_results "
                "WHERE quote_id=%s ORDER BY validation_id DESC LIMIT 1",
                (quote_id,),
            ).fetchone()
        if not row:
            return None
        return {
            ...
            "quote_id": quote_id,
            "status": row[1],
            "may_proceed": row[1] == "VALIDATED",
            "currency": row[2],
            "computed_subtotal": str(row[3]),
            "computed_vat": str(row[4]),
            "computed_total": str(row[5]),
            "issues": row[6],
            "policy": row[7],
            "claims": row[8],
    ...
    def list_for_rfq(self, rfq_ref: str) -> list[dict[str, Any]]:
        with psycopg.connect(self.dsn) as conn:
            rows = conn.execute(
                ...
                "q.validation_status,q.source_kind,q.created_at,vr.raw_sha256,vr.original_filename,"
    ...
    def list_open_reviews(self) -> list[dict[str, Any]]:
        with psycopg.connect(self.dsn) as conn:
            rows = conn.execute(
                "SELECT qr.review_id,qr.response_id,qr.quote_id,qr.opp_id,vr.rfq_ref,"
                "vr.original_filename,vr.raw_sha256,qr.error_code,qr.reason,qr.created_at "
