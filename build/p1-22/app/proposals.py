...
CREATE TABLE IF NOT EXISTS proposal_release_packages (
    release_package_id BIGSERIAL PRIMARY KEY,
    proposal_id        BIGINT NOT NULL REFERENCES proposals(proposal_id),
    build_job_id       BIGINT NOT NULL REFERENCES document_build_jobs(build_job_id),
    ...
    created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    CHECK (status IN ('READY_FOR_SUBMISSION','SUBMITTED','CANCELLED')),
    UNIQUE (proposal_id, build_job_id, package_sha256)
);
CREATE INDEX IF NOT EXISTS idx_proposal_release_packages_proposal
    ON proposal_release_packages (proposal_id, release_package_id DESC);

CREATE TABLE IF NOT EXISTS proposal_release_artifacts (
    release_artifact_id BIGSERIAL PRIMARY KEY,
    release_package_id  BIGINT NOT NULL REFERENCES proposal_release_packages(release_package_id),
    artifact_id         BIGINT NOT NULL REFERENCES proposal_artifacts(artifact_id),
    artifact_kind       TEXT NOT NULL,
    artifact_ref        TEXT NOT NULL,
    artifact_sha256     TEXT NOT NULL,
    metadata_json       JSONB NOT NULL DEFAULT '{}'::jsonb,
    UNIQUE (release_package_id, artifact_kind)
);

CREATE TABLE IF NOT EXISTS proposal_submissions (
    submission_id        BIGSERIAL PRIMARY KEY,
    release_package_id   BIGINT NOT NULL UNIQUE REFERENCES proposal_release_packages(release_package_id),
    ...
    recorded_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    notes                TEXT,
    CHECK (method IN ('MANUAL_EMAIL','PORTAL','COURIER','IN_PERSON','OTHER'))
);
"""
...
class ProposalRepository:
    ...
    def _release_inputs(self, conn: Any, proposal_id: int, build_job_id: int | None) -> dict[str, Any]:
        proposal = conn.execute(
            "SELECT p.proposal_id,p.opp_id,p.proposal_type,p.selection_id,p.status,"
            "pv.proposal_version_id,pv.payload_sha256 "
            "FROM proposals p JOIN proposal_versions pv ON pv.proposal_id=p.proposal_id "
            "WHERE p.proposal_id=%s AND pv.status='FROZEN' "
            "ORDER BY pv.version_no DESC LIMIT 1",
            (proposal_id,),
        ...
        selection = conn.execute(
            "SELECT proposal_eligible,deal_reg_status FROM quote_selections "
            "WHERE selection_id=%s AND status='ACTIVE'",
            (proposal[3],),
        ).fetchone()
        if not selection or not selection[0]:
            raise PermissionError("release requires the selected quote Deal Registration gate to be satisfied")
        approvals = self._approved_required_approvals(conn, proposal[1])
        if build_job_id is None:
            build = conn.execute(
                "SELECT build_job_id,state,builder_job_id,completed_at FROM document_build_jobs "
                "WHERE proposal_version_id=%s AND state='DONE' ORDER BY build_job_id DESC LIMIT 1",
                (proposal[5],),
            ).fetchone()
        else:
            build = conn.execute(
                "SELECT build_job_id,state,builder_job_id,completed_at FROM document_build_jobs "
                "WHERE build_job_id=%s AND proposal_version_id=%s",
                (build_job_id, proposal[5]),
            ).fetchone()
        if not build or build[1] != "DONE":
            raise PermissionError("release requires a completed document build job")
        validation = conn.execute(
            "SELECT passed,checks_json,created_at FROM document_validation_results "
            "WHERE build_job_id=%s ORDER BY validation_result_id DESC LIMIT 1",
            (build[0],),
        ).fetchone()
        if not validation or not validation[0]:
            raise PermissionError("release requires a passing document validation result")
        artifact_rows = conn.execute(
            "SELECT artifact_id,artifact_kind,artifact_ref,artifact_sha256,metadata_json,recorded_at "
            "FROM proposal_artifacts WHERE build_job_id=%s ORDER BY artifact_kind",
            (build[0],),
        ).fetchall()
        artifacts = [
            {
        ...
        if not {"docx", "pdf"}.issubset(kinds):
            raise PermissionError("release requires final DOCX and PDF artifacts")
        if any(not item["sha256"] for item in artifacts):
            raise PermissionError("release requires artifact SHA-256 hashes")
        return {
            "proposal": {
                "proposal_id": proposal[0],
                "opp_id": proposal[1],
                "proposal_type": proposal[2],
                "selection_id": proposal[3],
                "status": proposal[4],
                "proposal_version_id": proposal[5],
                "payload_sha256": proposal[6],
            },
            "build": {
                "build_job_id": build[0],
                "state": build[1],
                "builder_job_id": build[2],
                "completed_at": str(build[3]) if build[3] else None,
            },
            "approvals": approvals,
            "validation": {
                "passed": validation[0],
                "checks": validation[1],
                "created_at": str(validation[2]),
            },
            "artifacts": artifacts,
        }
    ...
    def prepare_release_package(
        ...
        *,
        ...
        actor = _clean_actor(actor)
        reviewer = _clean_actor(reviewed_by)
        with psycopg.connect(self.dsn) as conn:
            inputs = self._release_inputs(conn, proposal_id, build_job_id)
            package_payload = {
            ...
            package_hash = payload_sha256(package_payload)
            existing = conn.execute(
                "SELECT release_package_id,status,created_at FROM proposal_release_packages "
                "WHERE proposal_id=%s AND build_job_id=%s AND package_sha256=%s",
                (proposal_id, inputs["build"]["build_job_id"], package_hash),
            ).fetchone()
            if existing:
                ...
                release = conn.execute(
                    "INSERT INTO proposal_release_packages "
                    "(proposal_id,build_job_id,status,package_sha256,approval_snapshot,"
                    "artifact_snapshot,reviewed_by,review_comment,created_by) "
                    "VALUES (%s,%s,'READY_FOR_SUBMISSION',%s,%s,%s,%s,%s,%s) "
                    "RETURNING release_package_id,status,created_at",
                    (
                        proposal_id,
                        inputs["build"]["build_job_id"],
                        package_hash,
                        _json(inputs["approvals"]),
                        _json(inputs["artifacts"]),
                        reviewer,
                ...
                status = release[1]
                created_at = release[2]
                for artifact in inputs["artifacts"]:
                    conn.execute(
                        "INSERT INTO proposal_release_artifacts "
                        "(release_package_id,artifact_id,artifact_kind,artifact_ref,artifact_sha256,metadata_json) "
                        "VALUES (%s,%s,%s,%s,%s,%s)",
                        (
                            release_id,
                ...
                audit(
                    ...
                    actor,
                    "proposal_release",
                    "proposal_release_package_ready",
                    inputs["proposal"]["opp_id"],
                    new=_json(
                        {
                            "release_package_id": release_id,
                            "proposal_id": proposal_id,
                            "build_job_id": inputs["build"]["build_job_id"],
                            "package_sha256": package_hash,
                            "artifact_count": len(inputs["artifacts"]),
                        }
                    ),
                    reason=review_comment or None,
                )
                is_existing = False
            conn.commit()
        return {
    ...
    def record_submission(
        ...
        *,
        ...
        actor = _clean_actor(actor)
        submitted_by = _clean_actor(submitted_by)
        method = (method or "").strip().upper()
        if method not in {"MANUAL_EMAIL", "PORTAL", "COURIER", "IN_PERSON", "OTHER"}:
            raise ValueError("method must be MANUAL_EMAIL, PORTAL, COURIER, IN_PERSON or OTHER")
        if not recipient.strip() or not evidence_ref.strip() or not evidence_sha256.strip():
            raise ValueError("recipient, evidence_ref and evidence_sha256 are required")
        with psycopg.connect(self.dsn) as conn:
            package = conn.execute(
                "SELECT rp.release_package_id,rp.proposal_id,rp.status,rp.package_sha256,p.opp_id "
                "FROM proposal_release_packages rp JOIN proposals p ON p.proposal_id=rp.proposal_id "
                "WHERE rp.release_package_id=%s FOR UPDATE",
                (release_package_id,),
            ).fetchone()
            if not package:
                raise ValueError(f"unknown release package {release_package_id}")
            existing = conn.execute(
                "SELECT submission_id,method,recipient,evidence_ref,evidence_sha256,"
                "submitted_by,submitted_at,recorded_by,recorded_at,notes FROM proposal_submissions "
                "WHERE release_package_id=%s",
                (release_package_id,),
            ).fetchone()
            if existing:
                return {
                    ...
                    "proposal_id": package[1],
                    "opp_id": package[4],
                    "method": existing[1],
                    "recipient": existing[2],
                    "evidence_ref": existing[3],
                    "evidence_sha256": existing[4],
                    "submitted_by": existing[5],
                    "submitted_at": str(existing[6]),
                    "recorded_by": existing[7],
                    "recorded_at": str(existing[8]),
                    "notes": existing[9],
                    "existing": True,
                }
            if package[2] != "READY_FOR_SUBMISSION":
                raise PermissionError("release package is not ready for submission")
            submission = conn.execute(
                "INSERT INTO proposal_submissions "
                "(release_package_id,method,recipient,evidence_ref,evidence_sha256,"
                "submitted_by,recorded_by,notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) "
                "RETURNING submission_id,submitted_at,recorded_at",
                (
                    release_package_id,
                    method,
                    recipient.strip(),
            ...
            conn.execute(
                "UPDATE proposal_release_packages SET status='SUBMITTED',updated_at=now() "
                "WHERE release_package_id=%s",
                (release_package_id,),
            )
            conn.execute(
                "UPDATE opportunities SET status='SUBMITTED',updated_at=now() WHERE opp_id=%s",
                (package[4],),
            )
            audit(
                conn,
        ...
        return {
            ...
            "submitted_at": str(submission[1]),
            "recorded_by": actor,
            "recorded_at": str(submission[2]),
            "notes": notes or None,
            "existing": False,
        }

    def release_package(self, release_package_id: int) -> dict[str, Any]:
        with psycopg.connect(self.dsn) as conn:
            package = conn.execute(
                "SELECT rp.release_package_id,rp.proposal_id,rp.build_job_id,rp.status,"
                "rp.package_sha256,rp.approval_snapshot,rp.artifact_snapshot,rp.reviewed_by,"
                "rp.review_comment,rp.created_by,rp.created_at,p.opp_id,p.proposal_type "
                "FROM proposal_release_packages rp JOIN proposals p ON p.proposal_id=rp.proposal_id "
                "WHERE rp.release_package_id=%s",
                (release_package_id,),
            ).fetchone()
            if not package:
                raise ValueError(f"unknown release package {release_package_id}")
            submission = conn.execute(
                "SELECT submission_id,method,recipient,evidence_ref,evidence_sha256,"
                "submitted_by,submitted_at,recorded_by,recorded_at,notes "
                "FROM proposal_submissions WHERE release_package_id=%s",
                (release_package_id,),
            ).fetchone()
        return {
            "release_package_id": package[0],
            "proposal_id": package[1],
            "build_job_id": package[2],
            "status": package[3],
            "package_sha256": package[4],
            "approvals": package[5],
            "artifacts": package[6],
            "reviewed_by": package[7],
            "review_comment": package[8],
            "created_by": package[9],
            "created_at": str(package[10]),
            "opp_id": package[11],
            "proposal_type": package[12],
            "submission": None
            if not submission
            else {
                "submission_id": submission[0],
                "method": submission[1],
                "recipient": submission[2],
                "evidence_ref": submission[3],
                "evidence_sha256": submission[4],
                "submitted_by": submission[5],
                "submitted_at": str(submission[6]),
                "recorded_by": submission[7],
                "recorded_at": str(submission[8]),
                "notes": submission[9],
            },
        }
